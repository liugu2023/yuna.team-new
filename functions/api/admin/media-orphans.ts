...
async function collectReferencedMediaPaths(env: Env): Promise<Set<string>> {
  const [posts, siteRecords, backups] = await Promise.all([
    env.BLOG_DB.prepare("SELECT markdown_content, excerpt FROM posts").all<
      Pick<PostRecord, "markdown_content" | "excerpt">
    >(),
  ...
  for (const post of posts.results ?? []) {
    haystack.push(post.markdown_content || "", post.excerpt || "");
  }
